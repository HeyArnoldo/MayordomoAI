  ...
  buildSystemPrompt(locale: Locale, currency: string, userName?: string): string {
    ...
    if (locale === 'en') {
      return [
        ...
        '- For broad QUERIES ("my last transactions", "everything") do NOT ask for filters: call listTransactions without type or boxName. Filters are optional.',
        '',
        ...
        '- If a tool returns empty or an error, do NOT give up: retry with different parameters — widen the date range (current month, last 3 months), drop filters, or use another tool (getBoxBalances for allocations, getSpendingByBox for aggregates).',
